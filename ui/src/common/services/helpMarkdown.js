...
// `marked` resolves from the host app (peerDependency + Vite resolve.dedupe).
// Supersedes the per-app services/helpMarkdown.js forks.
//
// Rewrites:
//   - "foo.md" / "foo.md#section" → "/help/foo[#section]" with a
//     data-help-link attribute, so the drawer can intercept clicks and jump
//     within itself rather than navigating the SPA somewhere unknown.
//   - external "https?:" links get target=_blank + rel=noopener.
//   - every heading gets id="<slug>" (GitHub-style) so the drawer can
//     scrollIntoView for a given "foo.md#section" link.
...
// Strip the leading H1 since the host surface renders the title in its own
// header bar.
