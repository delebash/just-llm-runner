...
> ⛔ **SUPERSEDED FOR EXECUTION (2026-07-05) — this design is APPROVED + IN BUILD.** The live build tracker (single source of truth) is **`2026-07-05-model-surface-build.md`** — read THAT for phase status. The **§12 embedding-serving gap** that blocked the embedding half is **RESOLVED** (P3 of the SVM plan shipped co-resident embeddings). The "auto-pick-vs-pick-from-list" question flagged OPEN below was **RESOLVED in §15** (the speed-floor rule; QuickSetup stays one-model-auto-picked; `quality_rank` KEPT). **Phase 0 DONE** (runner `39d8712`): seed reconciled to the approved ladder — **Llama-3.3-70B (not 3.1)**; embeds nomic + qwen3-embedding-0.6b + bge-m3 + Qwen3-Embedding-8B — plus the explicit `embedding` catalog flag. This doc stays authoritative for the DESIGN (§1–15); the two review banners immediately below are HISTORY.
