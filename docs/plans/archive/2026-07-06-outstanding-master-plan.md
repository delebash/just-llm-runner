...
>
> **✅ THE EXECUTION VEHICLE CLOSED (2026-07-06 night): `archive/2026-07-06-model-per-hardware-plan.md` —
...
>
> Supersedes, as the outstanding-work view only: the open-item scatter across `archive/2026-06-28-MASTER-PLAN.md` (the roadmap archive — **fully folded 2026-07-08 via §I below**, so the master never needs to be opened as a tracker again), `archive/2026-06-28-ai-state-grid.md` (whose two ⚠ forks are now: per-model-tune-save → RESOLVED by Plan B; json-grammar → item C1 here), and the per-plan LIVE trackers (which stay authoritative for their own shipped history).
...
- **F1-a — `requestBlob` kit-only unify: DONE 2026-07-12** (rules-checker PASS; verified JW vitest 139/139 · JW+JV `build:vite` · JW headless smoke every-route errors=0 · DB byte-exact `0be0e2ef`). The kit had TWO `requestBlob`s — `client.js:65` path-first (THE public `@delebash/llm-ui` export, `index.js:14`) and a DEAD, shadowed method-first `serverApi.js:127`. Deleted the dead one + dropped `requestBlob` from `common/index.js:64`'s re-export → ONE path-first `requestBlob`. Zero runtime change (no direct importers; every consumer already resolved to client's path-first via the barrel or `../client.js`). **DEFERRED to the F1 JV integration (user, 2026-07-12: "we will fix jv when we do the integration"):** JV calls `requestBlob` METHOD-first at `projects.js:87,176` · `ExportPanel.vue:75` · `SettingsView.vue:155,1078` · `LinesView.vue:150` — already broken today (the barrel hands them path-first). ⚠ **AUTH divergence** (rules-checker's sharpest note): the surviving client `requestBlob`/`postForm` are AUTH-FREE; the deleted `serverApi` one carried Bearer `authHeaders`. JV authenticates on blob downloads, so the JV fix is NOT just arg-order — it needs an auth-capable path-first blob transport (serverApi's `authHeaders`/`configureServerApi.authToken` stay for exactly this). **`postForm` is in the SAME bucket** (identical dead-duplicate — `serverApi.postForm:137` authed vs `client.postForm:80` not; JV calls it for authed endpoints `projects.js:49,186`): converge `postForm` + fix its JV callers in the same F1 pass.
...
> **PROVENANCE.** Before bannering the 513 KB `archive/2026-06-28-MASTER-PLAN.md` fully historical (part
...
- **I1 — the JW cleanup tail (the master's T24 / Phase-F cleanup bucket).** STATUS:
  ...
  **THE JUDGMENT LEGS SHIPPED (2026-07-12, JW `5f1fa30`+`8491321`+`3f457f9`+`a575715`, post-rebase shas):**
  ...
  is committed at `justwrite-app/docs/plans/archive/2026-07-12-i1-css-popup-voicedrift.md`.
  ...
  **I1 TAIL — legs 1+2 SHIPPED 2026-07-21, leg 3 dropped:** (1) SettingsView's `.wb-*`
  ...
  broad composable a wash (the wrong-abstraction trap). The entity views + the 7 inline
  focus-watches STAY as-is. **REGROUNDED 2026-07-12:** a `text.test.js` now EXISTS
