...
**#119 (emitter pooling) — IN PROGRESS; the web-verification CHANGED the fix (do NOT blind-omit).** The box-finding's "omit `pooling=mean` — let the GGUF's `pooling_type` decide" is NOT safe. Web-verified 2026-07-04: llama.cpp uses the GGUF's baked-in `pooling_type` only IF present, else it errors ("failed to get embeddings from sequence, pooling type is not set" — abetlen/llama-cpp-python #1288) or uses a wrong default; **Qwen3-Embedding usage explicitly passes `--pooling last`** (its GGUF card + discussion #8; ggml-org/llama.cpp #14234 "bad output" + #20085 "all zeroes" are pooling-mismatch symptoms); and nomic-embed-text-v1.5's mean-pooling metadata could NOT be confirmed from its GGUF card. So the correct fix is **EXPLICIT per-model pooling** (nomic=`mean`, qwen3-embedding=`last`), NOT omission — which also SIDESTEPS the GGUF/box uncertainty entirely (an explicit `--pooling` always works, no b9644 guess needed). This is bigger than the planned one-line omit → surfaced to the user (rule #6): (A) pooling as a proper per-model DB attribute [configurable/editable] vs (B) a quick per-embed map + follow-ups. **User chose A (2026-07-04) + "do it right, professional, not lazy."** CORRECTED DESIGN (rule #7 — my first framing "per-model switch" was WRONG): pooling is NOT a switch. `resolve_model_switches` (switch_resolve.py:36-63) layers ONLY `all → type(moe|dense) → hardware` — its docstring states there is NO per-individual-model layer, so nomic + qwen3-embedding (both `dense`) CANNOT get different pooling via a switch. Pooling is an INTRINSIC per-model attribute → a **`ModelCatalog.pooling` String column**, mirroring the `use_limited` precedent (#74). Touch-list: `db.py` column; `CatalogRow.pooling` wire (model_catalog_api.py); `stores.py` `_catalog_to_wire` (:292) + upsert (:331); `seed.py` `pooling="mean"` on the nomic row + threaded into `seed_default_catalog` (qwen3-embedding=`last` lands with #120); `process.py:216` `ModelIniEntry.pooling` default `"mean"`→`""` (KEEP the field + its `emit_models_ini` embed-block render at :233-236); `lifecycle.py` `_resolve_ini_entries` sets `pooling=m.pooling` from the catalog in the main loop + the embed post-pass drops the hardcoded `pooling="mean"`; UI `useCatalogMeta.js` `poolingById` + `LuModelCatalog.vue` edit-form `UiSelect` (none/mean/cls/last/rank) mirroring the `useLimited` control → editable. Schema bump → dev reseed (fresh installs auto-create). Tests: UPDATE the two `pooling = mean` asserts (test_lifecycle.py:962, test_runner.py:257 — the embed entry now carries pooling explicitly) + a store round-trip + an emitter-reads-catalog test (pooling set → line present; unset → absent). **Pre-build rules-checker RUNNING on this plan; code only on PASS.**
