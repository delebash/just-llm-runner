    ...
    for i, k in enumerate(DEFAULT_KNOBS):
        ...
        if row is not None:
            if row.built_in:
                ...
                row.fit_relevant = bool(k.get("fit_relevant") or False)  # Phase 5 fingerprint set
                row.position = i
        ...
        s.add(db.KnobCatalog(
            ...
            backends=str(k.get("backends") or ""),
            fit_relevant=bool(k.get("fit_relevant") or False), position=i, built_in=True,
        ))
        s.flush()
