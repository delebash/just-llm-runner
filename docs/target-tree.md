# THE TARGET TREE — one skeleton for all three apps (EXECUTED)
...
Derived from the Round-2 audit (family-structure-audit.md).
...
| P11 | **DONE 2026-08-08** — the guard learns this page: **check 8** asserts the executed skeleton per app — server files (serve/app/app_state/paths/version/auth), NO per-app csrf.py, the §3b errors aliases actually re-exporting the platform (a regrown implementation fails), `<area>_api.py` naming with leading-underscore private helpers allowed, health_api + the `make_prefs_router` mount present, `database/` REQUIRED for JW+JV and FORBIDDEN for docgen (the ratified N/A is an assertion too), renderer lanes + the named skeleton files, KIT-DOOR CONTENT PINS (boot.smoke must ride `registerBootSmoke`, helpDocs `makeDocsHelpAdapter`, py.js the exec resolver — a door re-forked in place with its name intact now fails), no `__tests__/` dirs, per-app dev ports, `@renderer` in vite AND vitest, `biome check .` + caret-free biome pins, the family ruff pin; cross-app: biome.json BYTE-identical ×3 + one biome version; kit self: .gitattributes + the ruff pin. "Full family gates" read as: every gate's config asserted present family-wide (scripts, pins, configs) — the run battery itself stays the per-piece law. FIRST RUN CAUGHT A REAL P8 MISS: JV carried two tracked `__tests__` files (labContracts, kitTaskQueue) the JW-scoped P8 pattern never covered — flattened per P8 (one-level import strip, 28 vitest green), and `__tests__` is retired for JV too. One check-8 blind spot fixed in review: JW's py.js rides the kit through its two-hop door (smoke-common.js) — the pin accepts both ratified forms. *Audit/status sync*: this header PROPOSED→EXECUTED; the normative fold is app-structure.md **§14** (skeleton + config layer + the recorded-open divergences: vite 8-rolldown vs 6-classic, JW's settings/prefs deep split, docgen problem+json, the §3b alias sweep); family-structure-audit.md R2-STATUS closes with the program-executed stanza. Gates: guard ZERO with check 8 armed ×3 apps + kit self · JV (the one repo the piece touched beyond the kit): 28 vitest + lint + 409 pytest + build + smoke PASSED · JW/docgen untouched. | S |
