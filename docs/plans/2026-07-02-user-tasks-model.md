...
- **Cascade unchanged:** `FeaturePresetRef` (feature override) → `TaskKindPreset` (task's preset) → global default.
...
- **Extract `FeatureLab.vue`** — OWNS `draft`(read-only prompt)+`vars`+`loadSamplers`+samplers/switches+`columnConfig`+`<CompareStrip>`+`saveAs`/`delPreset`+a `use-production` EMIT + `productionPresetId` prop. 【NEW#1】 routing/pins/`saveRouting` stay in the PARENT; FeatureLab takes `pin` as a prop + emits pin changes.
...
- Routing/pins/saveRouting in the PARENT; every feature always HAS a task (reassignment only).
