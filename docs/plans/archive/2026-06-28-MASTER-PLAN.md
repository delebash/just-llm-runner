...
> **⛔ RECONCILIATION (2026-07-03, GGUF-grounded-model-layer Phase 6):** the 5 rows = the **4 JustWrite functions** (chat/prose/extract/analysis) **+ Attribution, a JustVoice task** (`speaker_attribution`; JW's `CLAUDE.md` bans speaker analysis — the attribution feature is JV §G). So the current app's JW routes **4 functions + `embed`** (the Phase-4 Models-grid columns `chat/prose/extract/analysis/embed`, with a per-app `other` bucket); the Attribution row informs **JV** only. **The per-tier picks are a 3-reviewer panel + user reasoning, NOT measured benchmarks (#28 — open);** **Tune & measure → Tasks Lab** (Phase 5 of `2026-07-02-gguf-grounded-model-layer.md`, shipped 2026-07-03) is how they get grounded per-machine. Full reconciliation note lives in `2026-06-27-model-catalog-research-and-recommendations.md` at this same matrix — the two copies are consistent.
