...
- **A3 — RAM-gated fit-filter (CODE FIX, panel-corrected — NOT a "confirm").** `coarse_fit` (`fit.py:91-105`) enforces `min_ram` ONLY on the CPU path (`vram_mb<=0`); the **GPU branch (L97+) checks VRAM only** → an 8 GB-VRAM / **16 GB-RAM** box is wrongly offered the 32 GB-RAM MoE. **FIX:** add the `min_ram_override` vs `ram_mb` check to `coarse_fit`'s GPU branch **AND** add a `ram_mb` param (+ system-RAM detect/override) to `get_models` (`runner/api.py:79`); `_fit` (`api.py:35`) passes both. **Accept:** 8 GB+16 GB-RAM → 35B-A3B/GLM-Air NOT offered; 8 GB+32 GB → offered. **Verify:** pytest (`test_runner_models.py`/`test_fit.py`).
...
- **D1 wire switch-override tables (panel-corrected):** `HardwareSwitch` ALREADY has a live reader (`switch_resolve.py:62`→`install.py:106`→`lifecycle.py:209`) → it needs a **WRITER/editor** (per-hardware switch editor), not a reader. `JobRouteSwitch` has a resolver (`switch_resolve.py:86`) + write API → only the **load-path caller** is unwired (🔒 residency). `PinSwitch` (per-feature, `db.py:230`) is the only **truly zero-reader** table → build store+resolver+reader.
...
- **D4 §6.6 finish:** confirm no stray switch UI remains in Providers (sub-editor removed already).
...
- **#31 (jobs-replace-role) — PARTIAL [IC]:** remove residual `quick`/`accuracy` from JW `routingBackend.js:15,55-56,78-79`.
