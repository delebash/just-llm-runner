...
The discussion above converged and the user approved the design on 2026-07-03 ("go ahead and update design in detail, wait to code"). This subsection SUPERSEDES the OPEN-items paragraph above: the **auto-pick rule (option a) is CONFIRMED**, the **auto-composed factual description is CONFIRMED**, QuickSetup stays **one-model-auto-picked**, and `quality_rank` is **dropped**. Two minor curation picks remain (flagged in §9). **No code is written until the user literally types "go."** Everything below was verified against real code and real GGUF files this session (not recalled).
