...
- **D1 wire switch-override tables (panel-corrected):** `HardwareSwitch` ALREADY has a live reader (`switch_resolve.py:62`→`install.py:106`→`lifecycle.py:209`) → it needs a **WRITER/editor** (per-hardware switch editor), not a reader. `JobRouteSwitch` has a resolver (`switch_resolve.py:86`) + write API → only the **load-path caller** is unwired (🔒 residency). `PinSwitch` (per-feature, `db.py:230`) is the only **truly zero-reader** table → build store+resolver+reader. *(Audit 2026-06-27: the JobRouteSwitch resolver `resolve_profile_switches` (`switch_resolve.py:69`) EXISTS but is currently UNCALLED — it awaits this orchestrator; it's intentional, don't re-discover it as "dead". `resolve_model_switches` IS wired into every load: `install.py:101-110`→`lifecycle.py:209`.)*
