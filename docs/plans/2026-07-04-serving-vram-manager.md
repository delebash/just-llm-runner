...
1. **⛔ GATING: `/v1/embeddings` routing in router mode** — confirm that a co-loaded embed entry (per-model `embeddings`/pooling in the `.ini`) serves `/v1/embeddings` routed by `model` id while the chat model serves `/v1/chat/completions`, with BOTH staying loaded (`--models-max 2`). **DEFINITIVELY env-blocked in this session** (attempted 2026-07-04: the org egress policy 403s ALL of GitHub — the API, `/releases/latest`, AND the release asset `…/releases/download/b9644/llama-b9644-bin-ubuntu-x64.tar.gz`; `/root/.ccr/README.md` forbids routing around a policy 403. HF model downloads DO work — the binary is the sole blocker. A prior session ran the CPU build under a different, more-permissive network policy.) **5-minute recipe on a real box** (where the app installs the engine normally): put a chat GGUF + a nomic GGUF on disk; write a models `.ini` with a chat entry (`model=…`, `ngl=999`, `c=4096`) and an embed entry (`model=…`, `embeddings=true`, plus the model's `pooling`); launch `llama-server --models-preset models.ini --models-max 2 --sleep-idle-seconds 300`; then `curl :8080/v1/chat/completions -d '{"model":"<chat>","messages":[…]}'` AND `curl :8080/v1/embeddings -d '{"model":"<embed>","input":"hi"}'`. PASS = the embeddings call returns a vector from the embed model while `GET /models` shows BOTH loaded. (Confirm exact `.ini` keys via `llama-server --help`.) If it FAILS → fall back to §11's dedicated second-embed-process, or Ollama/cloud embeddings.
