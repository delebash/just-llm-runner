...
> **⛔ 4b UPDATE (2026-07-05, doc-sync of a recorded USER decision): 4b is CLOSED-DROPPED, not deferred.** The model-surface LOCKED DECISIONS (recap, 2026-07-05) record the user's call — *"4b use Download/Set-as-default surface, leave"* — and the shipped Phase-2 catalog replaced Load/Unload with Download + Set-as-default / Set-as-embedding, which removes 4b's purpose (per-model residency controls). Nothing outstanding here. The line below is kept as history.
> **✅ STATUS 2026-07-04 — 4a SHIPPED + VERIFIED (user re-confirmed the go); 4b DEFERRED.** The user re-opened P4, asked the agent to re-ground + recommend, and after the agent recommended "build 4a now, defer 4b" the user chose **"do 4a defer 4b"** and then gave the explicit **"go"** (2026-07-04) — the fresh re-confirmation this hold required (recorded here so the SSOT matches reality, matching how P1/P2/P3 each carry their "user 'go'"). 4a is BUILT + VERIFIED (see the VERIFIED note at the end of this spec); **4b (the `LuModelCatalog`/`useRunnerModels` resident-awareness) stays DEFERRED** — not built in this task. The earlier agent OVERSTEP (an `AskUserQuestion` that invented a "sibling panel" and re-opened the plan-decided `LuRunnerEngine.vue` placement — rule #6 cannot-override-design-docs + rule #9 no-own-decisions) stays RETRACTED: the resident view lives IN `LuRunnerEngine.vue`, there is NO sibling. Full audit: `justwrite-app/MORNING_RECAP.md` §"RESUME — DECISIONS AUDIT + P4 OVERSTEP."
>
> **4a BUILD SPEC (pre-build rules-checker returned FAIL(2) → both folded here, 2026-07-04):**
> 1. **T5a — render the WHOLE resident set, not just loaded/sleeping.** `resident()` (`llm_runner/runner/lifecycle.py:582-608`) emits the router's live statuses (`loaded|sleeping|unloaded|…`) AND, from `self._resident` (`:602-607`), in-flight/error rows (`downloading|starting|error`) that the router can't see yet — e.g. an `engine-not-installed` failure. Those in-flight/error rows carry ONLY `id`/`status`/`vram_mb` (no `n_ctx`/`n_params`/`size_bytes`). The view must render EVERY status verbatim and TOLERATE meta-less rows, so a failed model stays VISIBLE (never hidden by a loaded/sleeping-only render).
> 2. **T3 build-guard — the knob-save PUT sends ONLY `{modelsMax, sleepIdleSeconds}`.** `EngineConfigUpdate` is all-optional and the handler sets each field only if present, so a partial PUT is safe — but the resident-view save must NOT echo a full config, or it clobbers `binaries`/`pinnedBuild`/`safetyMarginMb`. Mirror `LuRunnerBinaries.saveSettings` (`ui/src/components/LuRunnerBinaries.vue:74-76`) which sends only its two fields.
> 3. **T7 build-guard — `str()`-wrap the two reset defaults** in `reset_to_defaults` (`("models_max", str(DEFAULT_MODELS_MAX))`, `("sleep_idle_seconds", str(DEFAULT_SLEEP_IDLE_SECONDS))`) to match the string column + the existing `safety_margin_mb` pattern (`stores.py:859`); and refresh the `runner_config_api.py:2-6` MODULE docstring to list the two knobs, not only the `:47` protocol comment.
> 4. **T2 correction — JV does NOT mount this UI.** A grep of `JustVoice/src/renderer/src` finds ZERO of `LuRunnerEngine`/`LuModelCatalog`/`useRunnerModels` (JV uses its own local `ProviderForm`), and JV mounts neither `make_runner_config_router` nor `install_llm` (`JustVoice/server/justvoice/app.py:190-200`) so it never serves `/v1/ai/engine-config`. So the 4a UI change does NOT touch JV. JV-safety verify for 4a = `import llm_runner.llm.{runner_config_api,stores}` + grep-for-removed-symbols (purely additive — none), NOT a JV boot (known-red on the `LLMRolesSettings` drift, `:456`; out of scope). The JV UI smoke stays reserved for 4b.
> 5. **T5b — PIN the §4a help/user-doc entry:** JW `services/helpDocs.js` + a `docs/*.md` (per the P3 `justwrite-app/docs/ai-providers.md` precedent) with PLAIN-language copy — describe "how many models stay loaded at once" + "how long an idle model waits before unloading," never the raw `models_max`/`TTL` jargon.
>
> **AS BUILT (2026-07-04):** BACKEND (runner) — `llm/runner_config_api.py`: `EngineConfig` + `EngineConfigUpdate` gain `modelsMax`/`sleepIdleSeconds`; the PUT persists them via the existing `set_setting` with clamps (`models_max = max(1, …)`, `sleep_idle_seconds = max(0, …)`); module docstring + the `:47` comment refreshed. `llm/stores.py`: `get_config()` returns the two from `cfg`; `reset_to_defaults()` resets them (str-wrapped, from `DEFAULT_MODELS_MAX`/`DEFAULT_SLEEP_IDLE_SECONDS`). `tests/test_runner_config_store.py`: +5 tests (exposes / persists / clamp-up-to-1 + preserve-zero-TTL / partial-PUT-no-clobber / reset). UI (shared kit) — `ui/src/components/LuRunnerEngine.vue`: a 2nd `usePoll(refreshResident, 2500)` on `/v1/llm-runner/resident`; a "Loaded models" section rendering the VRAM budget + EVERY status verbatim (tolerating meta-less error/in-flight rows) + the two knob editors (`UiInput type=number`, save sends ONLY the two fields, re-syncs the clamps). JW — `docs/ai-providers.md` §"Loaded models & GPU memory" (plain-language) + `scripts/resident-panel-probe.mjs` (the mock-based render probe).
>
> **VERIFIED (container, 2026-07-04):** runner `ruff` clean + **297 pytest** (+5 new); JW `build:vite` clean + headless smoke **0 JS errors** (provider-form `engine=true`); `resident-panel-probe.mjs` PASSED (section renders · loaded+sleeping+**error** all shown · meta-less row tolerated · VRAM budget renders · knobs seed from `/resident` · Save sends ONLY `{modelsMax, sleepIdleSeconds}`); live curl (GET/PUT `/v1/ai/engine-config` exposes+persists the knobs, `modelsMax:0`→clamped to 1, `sleepIdleSeconds:0` preserved, binaries/pinnedBuild un-clobbered; GET `/v1/llm-runner/resident` reflects the edit). JV-safe: both changed modules import; JV grep=0 for `LuRunnerEngine`/`LuModelCatalog`/`useRunnerModels` (renderer) + `make_runner_config_router`/`install_llm`/`engine-config` (server) — JV serves none of the changed surface (JV boot NOT used — known-red on `LLMRolesSettings`, out of scope). Diff rules-checker: PASS.
**4a.** `LuRunnerEngine.vue`: add a "resident models" view (the FULL resident set — every status, per BUILD SPEC 1
above — + the VRAM budget + `models_max` + TTL, reading `/v1/llm-runner/resident`); edit the two DB-backed knobs
there. Add a help/user-doc entry for the two operator knobs + residency (panel T11).
...
**PIN RECONSIDERATION (the P3 deviation, now a USER DECISION — do NOT decide unilaterally, rule #6):** the user leans toward "leave the embed IN the `.ini`" rather than P3's lazy `ensure_embedding`. Grounding: `models-autoload` DEFAULTS to enabled — the router auto-loads a model when a request for it arrives, so "embed in the `.ini`, loads when needed" works ONCE THE ROUTER IS UP. The two things the bare-`.ini` approach loses vs P3-lazy: (1) something must still SPAWN the router if RAG runs before any chat (a `.ini` alone doesn't self-start); (2) an auto-loaded embed is INVISIBLE to the arbiter's VRAM ledger (the exact "unreserved resident" hole that made P3 drop `load-on-startup`). **The clean reconciliation IF the user chooses eager:** mark the embed section `load-on-startup = true` AND have the runner RESERVE it pinned when it SPAWNS the router (closes the unreserved-resident hole) — keep a thin ensure only to spawn-the-router-if-down for the RAG-first case. That gives the user's "it's just in the ini, always resident" mental model WITH correct VRAM accounting.

**✅ DECIDED (USER): EAGER.** Recorded 2026-07-05 after the user corrected the tracker: *"i did make decision embedding to eager you missed it"* — the decision HAD been made and this doc wrongly still carried it as "pending" (the recording failure is on the agent side; the earlier P3-era "lazy" note at `:446` reflects the FIRST decision, which the user later reversed in this reconsideration — the reversal is the one that stands). **The build spec = the clean reconciliation above, verbatim:** the embed `.ini` section gets `load-on-startup = true` + the runner RESERVES the embed pinned when it spawns the router (keeping the arbiter ledger exact — this closes both hazards that motivated the lazy as-built: the invisible auto-load and the 400-on-re-ensure) + `ensure_embedding` slims to spawn-the-router-if-down for the RAG-before-any-chat case. **Code today is still LAZY** (verified 2026-07-05: `lifecycle.py:921` deliberately omits `load-on-startup`; `ensure_embedding` at `:618` is the loader) — so this is an OUTSTANDING BUILD ITEM awaiting the user's "go"; the `ModelIniEntry.load_on_startup` emitter capability it needs already exists (unit-tested, kept for exactly this).
...
| 64 GB | Meta-Llama-3.1-70B-Instruct (Qwen3-72B DROPPED in #120 — not an official Qwen model, only community upscales) | Q6_K | ~58.5 GB | 64000 | qwen3-embedding-0.6b | 16384 | 80000 / 16384 / `cls` |
