...
function keyOf(feature, action) {
  return `${feature || ""} ${action || ""}`;
}

async function fetchRoute(feature, action) {
  const q = new URLSearchParams({ feature });
  if (action) q.set("action", action);
  return request(`/v1/ai/resolved-route?${q}`);
}

/** The cached row for (feature, action), or null before the first fetch. */
function routeFor(feature, action = "") {
  return cache[keyOf(feature, action)] || null;
}

/** Fetch-if-absent. Coalesces concurrent calls per key. */
function ensureRoute(feature, action = "") {
  if (!feature) return Promise.resolve(null);
  const key = keyOf(feature, action);
  if (cache[key]) return Promise.resolve(cache[key]);
  return refreshRoute(feature, action);
}

/** Force a re-fetch (a routing/preset edit elsewhere invalidates us). */
function refreshRoute(feature, action = "") {
  if (!feature) return Promise.resolve(null);
  const key = keyOf(feature, action);
  if (inflight.has(key)) return inflight.get(key);
  const p = fetchRoute(feature, action)
    .then((row) => {
      cache[key] = row || null;
      return cache[key];
    })
