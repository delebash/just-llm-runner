...
## 7 · Open decisions (the user's call — surfaced, NOT decided by the agent)
1. **Co-residence policy defaults:** always keep the (tiny) embed model resident? Default idle TTL for chat models? On a big card, keep 2 chat models or reserve headroom for TTS?
2. **JV coordination mechanism:** how the shared LLM router and JV's `engines/manager.py` share one VRAM ledger — one arbiter process both consult, an IPC/HTTP budget endpoint, or a shared on-disk ledger? (JV keeps its own TTS engines; only the budget view is shared.)
3. **Where the arbiter lives:** in `just-llm-runner` (shared, both apps consume) — assumed, confirm.
4. **`.ini` emission mapping:** the exact `switch_presets` → per-model `.ini` key mapping (which flags, short vs long forms).

## 8 · Open verifications (runtime / build-time — NOT closable in this container)
1. **⛔ GATING: `/v1/embeddings` routing in router mode** — confirm on a real box that a co-loaded embed entry (with per-model `embeddings` in the `.ini`) serves `/v1/embeddings` routed by `model` id while the chat model serves `/v1/chat/completions`. Env-blocked here (GitHub download denied → no `llama-server`; llama-cpp-python's wrapper doesn't expose router mode). If it FAILS, fall back to §11's second-embed-process or Ollama/cloud embeddings.
