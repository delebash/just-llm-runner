...
- **Platform settings remainder [IC]:** U4 Updates/Changelog — *partial (audit 2026-06-27): the shared `UpdatesPanel.vue` component EXISTS (exported `index.js:27`) but is **not mounted** in any settings view; JW's `WhatsNewModal.vue` IS live (`App.vue:17,131`)* → remaining = mount the shared panel in Settings. Cache/Data "reclaim disk"; generic Hardware panel in the AI menu (both apps).
